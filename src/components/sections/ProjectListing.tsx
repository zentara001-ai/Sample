            ...
            <section className="py-32 container mx-auto px-6">
                ...
                >
                    {displayedProjects.map((project, i) => (
                        ...
                        >
                            <Card className="h-full overflow-hidden transition-all duration-500 group border-border/50 bg-background/50 backdrop-blur-sm flex flex-col hover:border-primary/50 hover:shadow-[0_0_30px_-10px_rgba(0,0,0,0.3)] dark:hover:shadow-[0_0_30px_-10px_rgba(255,255,255,0.1)]">
                                ...
                                <CardHeader>
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <p className="text-primary text-xs font-bold uppercase tracking-widest mb-1 opacity-70 group-hover:opacity-100 transition-opacity">{project.category}</p>
                                            <CardTitle className="text-2xl group-hover:text-primary transition-colors">{project.title}</CardTitle>
                                        </div>
                                    </div>
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        {project.technologies?.slice(0, 3).map(tech => (
                                            <span key={tech} className="text-xs font-medium px-2 py-1 rounded-full bg-secondary text-secondary-foreground border border-border/50 group-hover:border-primary/30 transition-colors">
                                                {tech}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <CardContent className="flex-grow flex flex-col justify-between">
                                    <CardDescription className="text-base line-clamp-3 mb-4 group-hover:text-foreground transition-colors">
                                        {project.description}
                                    </CardDescription>
                                    <div className="pt-4 border-t border-border/40 flex items-center justify-between text-sm text-muted-foreground group-hover:text-primary/80 transition-colors">
